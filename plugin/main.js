 ...
 async function upload(selection) {
    ...
    return;
      ...
      if (img.byteLength==0){
        ...
        console.log("--------------------srever");
        const photoUrl = "http://127.0.0.1:5000/upload";
        data = await xhrBinary(photoUrl);
        console.log(data)
        await generateLatout(selection);
 ...
 function getDialog() {
  ...
  function createDialog() {
    ...
    document.body.innerHTML = `
      ...
      <dialog>
          <form method="dialog">
              <h1><span>Sketch2Wireframe</span><img class="icon" src="images/icon@1x.png"></h1>
              <hr />
              <h2><span>Add a hand-drawn sketch</span></h2>
              ...
              <div id="upload" class="ai-upload-zone" data-hover="false" data-disabled="false" style="width: 100%; height: 280px;">
                ...
                <div class="ai-upload-zone-main-tip">upload a picture from your computer</div>
              </div>
              <footer>
...
async function clearArtboard(selection) {
  // 获取选定的画布
  ...
  if (artboard) {
    // 获取画布上的所有元素
    const allElements = artboard.children;
    ...
    console.log("No artboard selected!");
  }
}

async function draftCommand(selection){
  // 先清空画布
  await clearArtboard(selection);
  const photoUrl = "http://127.0.0.1:5000/draft"; // 使用相应的URL
  data = await xhrBinary1(photoUrl);
  console.log(data);
  await generateLatout(selection);
}
  
async function heuristicsCommand(selection){
  // 先清空画布
  await clearArtboard(selection);
  const photoUrl = "http://127.0.0.1:5000/heuristics"; // 使用相应的URL
  data = await xhrBinary1(photoUrl);
  console.log(data);
  await generateLatout(selection);
  }

async function ipOptimizerCommand(selection){
  // 先清空画布
  await clearArtboard(selection);
  const photoUrl = "http://127.0.0.1:5000/ipOptimizer"; // 使用相应的URL
  data = await xhrBinary1(photoUrl);
  console.log(data);
  await generateLatout(selection);
}

async function generateLatout(selection){
  let root = data[0]
...
async function gatherAndSendData(selection) {
  const selectedItems = selection.items;

  const elementsData = selectedItems.map(item => {
    // 获取元素的位置信息
    const { x, y } = item.globalBounds;
    // 获取元素的宽度和高度
    const { width, height } = item.localBounds;
